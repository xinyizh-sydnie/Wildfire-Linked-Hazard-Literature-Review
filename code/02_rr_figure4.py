"""Relative-risk analysis and Figure 4 / Supplementary Figure D1. Reads
data/hotspot_50km/, study_table.csv, the GRDI raster, and the 10 km WUI rasters;
writes figures to outputs/figure4/. Imports srex_regions.py. See SI C3-C4, D."""
...
DATA_10KM_DIR    = DATA_DIR / "land_10km"
...
def _dersimonian_laird_tau2(logrr_arr, se_arr):
    ...
    w = 1.0 / (se_arr ** 2)
    w_sum = np.sum(w)
    mu_fe = np.sum(w * logrr_arr) / w_sum
    Q = np.sum(w * (logrr_arr - mu_fe) ** 2)
    k = len(logrr_arr)
...
def compute_regional_relative_risk_yearly(compounds, fires, compound_type='HOT_DRY',
    ...
    n_exposed_global = int((comp_hot_global & global_domain).sum())
    ...
    for y in years:
        ...
        a = int(np.sum(fire_hot & comp_hot_global & global_domain))
        b = int(np.sum((~fire_hot) & comp_hot_global & global_domain))
        c = int(np.sum(fire_hot & (~comp_hot_global) & global_domain))
        d = int(np.sum((~fire_hot) & (~comp_hot_global) & global_domain))
        ...
        if HAS_REGIONMASK:
            for ridx in region_ids:
                ...
                rows.append({
                    ...
                    "significant": sig,
                    "n_cells": int(reg_mask.sum()),
                    "n_exposed": int((comp_hot_regional & reg_mask).sum()),
                    'a': a, 'b': b, 'c': c, 'd': d,
                })
...
if WUI_DIR.exists():
    ...
    if wui_fraction is not None and urban_fraction is not None:
...
for ct in sorted(df_comp["hazard"].unique()) if not df_comp.empty else []:
    ...
    ax.scatter(
        ...
        s=sub["s"].values,
        marker=COMPOUND_MARKERS.get(ct, "o"),
...
sm_srex_grid = np.array(_sm_srex_flat).reshape(lon2d.shape)
...
# Regime categories follow Kelley et al. 2019 (Fig 2a), independent of the RR results
REGIME_CATEGORY = {
    'C. Asia': 'fuel',
    'W. Asia': 'fuel',
