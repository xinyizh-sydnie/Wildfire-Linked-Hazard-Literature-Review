"""
LLM Literature Classifier v7 - Fuzzy Voting with Checkpoints

Classifies academic papers related to wildfire hazard research using a
multi-prompt LLM pipeline with majority voting for output stability.

Key features:
  - 5x majority voting per paper with fuzzy label matching
  - Semantic normalization of equivalent terms (e.g., bushfire -> wildfire)
  - Fixed seed + top_p=1 for maximum reproducibility
  - Automatic checkpoint saving and resume capability

Requirements:
  pip install "openai>=1.109.0" pandas==2.2.2 tqdm openpyxl xlsxwriter

Usage:
  Set OPENAI_API_KEY environment variable, configure INPUT_PATH and OUTDIR,
  then run: python llm_classifier.py
"""
...
# =============================================================================
# CONFIGURATION
# Set these variables before running. INPUT_PATH and OUTDIR can also be passed
...
# Input CSV file containing paper metadata (title, abstract, keywords, etc.)
...
# Optional path to an existing progress file for resuming a previous run
RESUME_FROM_FILE = os.environ.get("RESUME_FROM_FILE", "")
...
# Majority voting: each paper is processed this many times; majority label wins
VOTING_RUNS = 5
VOTING_FIELDS = [
...
# Fuzzy matching: values with similarity above this threshold are treated as equal
FUZZY_THRESHOLD = 0.6
...
# Derived paths (set after OUTDIR is confirmed)
CHECKPOINT_FILE = os.path.join(OUTDIR, "checkpoint_progress.csv")

# Runtime flags (modify here or override before calling process_dataset_with_checkpoints)
TEST_MODE = False
TEST_PAPERS = 10
USE_VOTING = True
...
CLOSED_SETS = {
    ...
    "setting": {"WUI", "peri-urban", "urban", "rural", "mixed/unclear"},
    "scale": {
...
TASK: Extract geographic locations AND estimate bounding box coordinates.
...
- Only use null for MOB if location is too vague to estimate""")
...
def process_paper_single_run(row: pd.Series, run_seed: int = SEED) -> Dict[str, Any]:
    """Run all 6 prompts on a single paper and return a combined result dict."""
    ...
    # Prompt 2: Hazard classification
...
def fuzzy_majority_vote(values: List[str], field_name: str = "") -> Tuple[str, float, List[str], bool]:
    """Return (winner, agreement_ratio, all_votes, had_fuzzy_merge)."""
    if not values:
...
def process_paper_with_voting(row: pd.Series, paper_num: int = 0) -> Dict[str, Any]:
    """Process a paper VOTING_RUNS times and return the majority-voted result."""
...
class CheckpointManager:
    """Manages saving and loading of intermediate results for resumable runs."""
    ...
    def load_existing_progress(self, resume_file: str = None) -> int:
        """Load prior results; return the count of already processed papers."""
        files_to_check = []
        if resume_file and os.path.exists(resume_file):
            files_to_check.append(resume_file)
        if os.path.exists(self.checkpoint_file):
            files_to_check.append(self.checkpoint_file)

        batch_files = glob.glob(os.path.join(self.output_dir, "batch_up_to_*.csv"))
        batch_files.sort(
            key=lambda x: int(re.search(r"batch_up_to_(\d+)\.csv$", x).group(1))
            if re.search(r"batch_up_to_(\d+)\.csv$", x) else 0,
            reverse=True,
        )
        files_to_check.extend(batch_files)

        results_files = glob.glob(os.path.join(self.output_dir, "results_*.csv"))
        ...
        for fpath in files_to_check:
            ...
            try:
                ...
                if len(df) > 0 and "article_id" in df.columns:
                    if loaded_df is None or len(df) > len(loaded_df):
                        loaded_df = df
                        loaded_from = fpath
            except Exception as e:
        ...
        if loaded_df is not None:
            self.results = loaded_df.to_dict("records")
            self.processed_ids = {
                str(r.get("article_id", "")) for r in self.results if r.get("article_id")
            }
            logger.info(f"Loaded {len(self.results)} existing results from {loaded_from}")
    ...
    def save_checkpoint(self, force: bool = False, papers_since_last: int = 0) -> bool:
        ...
        if force or papers_since_last >= CHECKPOINT_EVERY_N:
            try:
                ...
                backup_path = os.path.join(self.output_dir, f"batch_up_to_{len(self.results)}.csv")
                df.to_csv(backup_path, index=False)
                logger.info(f"Checkpoint saved: {len(self.results)} papers -> {backup_path}")
                return True
            except Exception as e:
...
def process_dataset_with_checkpoints(
    ...
    """
    Classify all papers in input_path, saving checkpoints along the way.

    Args:
        input_path:  Path to the input CSV file.
        output_dir:  Directory for output and checkpoint files.
        resume_file: Optional path to an existing results file to resume from.
        use_voting:  If True, run each paper VOTING_RUNS times and take majority vote.
        max_papers:  Optional cap on the number of new papers to process.

    Returns:
        DataFrame containing all results (including any pre-existing ones).
    """
    os.makedirs(output_dir, exist_ok=True)
    print("=" * 60)
    print("STARTING PROCESSING WITH CHECKPOINTS")
    print("=" * 60)

    ckpt = CheckpointManager(output_dir)
    existing_count = ckpt.load_existing_progress(resume_file)
...
# =============================================================================
# UTILITY: MERGE BATCH FILES
# =============================================================================

def merge_all_batches(output_dir: str) -> Optional[pd.DataFrame]:
    """
    Merge all batch_up_to_*.csv files in output_dir into a single deduplicated file.
    Useful for consolidating interrupted runs before resuming.
    """
    batch_files = glob.glob(os.path.join(output_dir, "batch_up_to_*.csv"))
    if not batch_files:
    ...
    for f in batch_files:
        try:
            df = pd.read_csv(f)
            new_papers = df[~df["article_id"].astype(str).isin(seen_ids)]
            if len(new_papers) > 0:
                all_dfs.append(new_papers)
                seen_ids.update(new_papers["article_id"].astype(str).tolist())
        except Exception as e:
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    merged_path = os.path.join(output_dir, f"merged_all_{timestamp}.csv")
    merged.to_csv(merged_path, index=False)
    print(f"Merged {len(merged)} unique papers from {len(batch_files)} batch files")
    print(f"Saved to: {merged_path}")
    return merged


def check_progress(output_dir: str, checkpoint_file: str, resume_file: str = ""):
    """Print a summary of all saved progress files in output_dir."""
    print("Checking saved progress...\n")

    if os.path.exists(checkpoint_file):
        df = pd.read_csv(checkpoint_file)
        print(f"Checkpoint file: {len(df)} papers ({checkpoint_file})")
    else:
        print("No checkpoint file found")

    if resume_file and os.path.exists(resume_file):
        df = pd.read_csv(resume_file)
        print(f"\nResume file: {len(df)} papers ({resume_file})")
    elif resume_file:
    ...
    if batch_files:
        ...
        print(f"\nFound {len(batch_files)} batch files (last 5):")
    ...
    if results_files:
        results_files.sort(key=os.path.getmtime)
        print(f"\nFound {len(results_files)} results files (last 3):")
        for f in results_files[-3:]:
            df = pd.read_csv(f)
            print(f"  {os.path.basename(f)}: {len(df)} papers")
    else:
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("RESUMABLE LLM CLASSIFIER")
    print("=" * 60)
    print(f"Input:            {INPUT_PATH}")
    print(f"Output dir:       {OUTDIR}")
    print(f"Resume from:      {RESUME_FROM_FILE or '(none)'}")
    print(f"Checkpoint every: {CHECKPOINT_EVERY_N} papers")
    print(f"Voting:           {'ON (' + str(VOTING_RUNS) + 'x)' if USE_VOTING else 'OFF'}")
    print(f"Fuzzy matching:   {'ON (threshold=' + str(FUZZY_THRESHOLD) + ')' if USE_FUZZY_MATCHING else 'OFF'}")
    if TEST_MODE:
        print(f"TEST MODE:        Processing only {TEST_PAPERS} new papers")
    print("=" * 60 + "\n")

    results_df = process_dataset_with_checkpoints(
        input_path=INPUT_PATH,
        output_dir=OUTDIR,
        resume_file=RESUME_FROM_FILE or None,
        use_voting=USE_VOTING,
        max_papers=TEST_PAPERS if TEST_MODE else None,
    )
